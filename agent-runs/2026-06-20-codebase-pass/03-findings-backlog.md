...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| F-001 | P1 | Bug/Security | Fixed | Sign-in redirect | Raw `next` query values flowed into client navigation after sign-in. | `src/app/signin/page.tsx` now normalizes `next` to same-origin protected app paths before passing it to the client; `./node_modules/.bin/eslint .` and `npm run build` passed. | Unsafe navigation/open redirect; Next warns against unsanitized router URLs. | Small | Build, ESLint, source inspection; invalid external/javascript `next` falls back to `/dashboard`. | Done in T-004. |
| F-002 | P1 | Package update | Partially fixed/deferred | Runtime dependencies | Production audit reported 7 vulnerabilities, including high-severity Next.js advisories and transitive Firebase/Admin dependencies. | `npm update` reduced audit to 2 moderate advisories through Next's nested PostCSS dependency; `npm audit fix --force` would install a breaking Next downgrade, so it is deferred. | Security/reliability exposure in app framework and transitive packages. | Medium | `npm audit`, build, ESLint. | Monitor/update when Next publishes a non-breaking fix. |
| F-003 | P1 | Package update | Fixed | Install integrity | Local install was out of sync with `package.json`, with invalid root dependencies and extraneous packages. | `npm ci`, `npm ls --depth=0`, `npm run lint`, and `npm run build` now pass after lockfile update. | Builds may use different package versions than declared; cleanup changes may be hard to reason about. | Small/Medium | `npm ls --depth=0`, build, ESLint. | Done in T-005. |
| F-004 | P2 | Bug/Reliability | Fixed | Auth/session consistency | If Firebase popup succeeded but `/api/auth/session` failed, Firebase client auth remained signed in while no server session existed. | `src/stores/auth-store.ts` now clears server session cookies and Firebase client auth after session creation failure; `./node_modules/.bin/eslint .` and `npm run build` passed. | User could get stuck in a signed-in client state that protected server/proxy routes rejected. | Small | Build, ESLint, manual code inspection. | Done in T-009. |
...
| F-006 | P2 | Dead code | Fixed | Store public surface | `calculateGoalProgress` and `updateGoalStatus` were exported store methods with no call sites. | Methods were removed from `src/stores/app-store.ts`; `rg "calculateGoalProgress|updateGoalStatus" src` finds no references; lint/build pass. | Larger public surface and extra write paths that are not exercised. | Small | Remove methods; build and ESLint pass. | Done in T-010. |
| F-007 | P2 | Test gap | Partially fixed/deferred | Quality scripts | Package had ESLint deps/config but no `lint`, `test`, or `typecheck` scripts. | `package.json` now defines `lint`; `npm run lint` and `npm run build` pass. Test/typecheck scripts remain deferred because no test framework exists and build already runs TypeScript. | Harder for future agents/CI to run consistent gates. | Small | Add `lint` script after package integrity is repaired; build/ESLint. | Lint fixed; tests deferred. |
